...
test("matchScore: issue_number matches Issue.number (the core case this project exists for)", () => {
  const leafFreq = buildLeafFrequency(new Map([["X", [outField("number", "Issue")]]]));
  const score = matchScore(input("issue_number"), outField("number", "Issue"), leafFreq);
  assert.equal(score, 5);
});

test("matchScore: pull_number matches PullRequest.number", () => {
  const leafFreq = buildLeafFrequency(new Map([["X", [outField("number", "PullRequest")]]]));
  const score = matchScore(input("pull_number"), outField("number", "PullRequest"), leafFreq);
  assert.equal(score, 5);
});
...
test("matchScore: wrong owning type is rejected even with a matching leaf name", () => {
  ...
  const leafFreq = buildLeafFrequency(new Map([["X", [outField("number", "Milestone")]]]));
  const score = matchScore(input("pull_number"), outField("number", "Milestone"), leafFreq);
  assert.equal(score, 0);
});

test("matchScore: leaf name not contained in input name scores 0", () => {
  const leafFreq = buildLeafFrequency(new Map([["X", [outField("owner", "Repo")]]]));
  const score = matchScore(input("issue_number"), outField("owner", "Repo"), leafFreq);
  assert.equal(score, 0);
});

test("matchScore: exact match with a rare (non-generic) field name scores 4", () => {
  const leafFreq = buildLeafFrequency(new Map([["X", [outField("sha", "Commit")]]]));
  const score = matchScore(input("sha"), outField("sha", "Commit"), leafFreq);
...
test("matchScore: exact match with a generic (very common) field name scores 1, below threshold", () => {
  ...
  const outputsByTool = new Map(
    Array.from({ length: 30 }, (_, i) => [`TOOL_${i}`, [outField("id", "Whatever")]]),
  );
  const leafFreq = buildLeafFrequency(outputsByTool);
  const score = matchScore(input("id"), outField("id", "Whatever"), leafFreq);
...
test("isGeneric: true once a leaf name is produced by more tools than the threshold", () => {
  const outputsByTool = new Map(
    Array.from({ length: 26 }, (_, i) => [`TOOL_${i}`, [outField("name", "Whatever")]]),
  );
  const leafFreq = buildLeafFrequency(outputsByTool);
...
test("isGeneric: false for a rare field name", () => {
  const leafFreq = buildLeafFrequency(new Map([["TOOL_0", [outField("node_id", "Whatever")]]]));
